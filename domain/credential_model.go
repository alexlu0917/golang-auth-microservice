...
func NewCredential(params SaveParams) (Credential, error) {
	...
	return Credential{ID: params.ID, Name: params.Name}, nil
}
...
func validate(params SaveParams) error {
	...
	if params.Password != "" && len(params.Password) < 8 {
		return fmt.Errorf("password must be at lease 8 charaters")
	}
...
func (a *Credential) GenerateAuthToken() error {
	...
	a.AuthToken = AuthToken{ID: AuthTokenID(hex.EncodeToString(h.Sum(nil))), ExpiresAt: time.Now().Add(24 * time.Hour)}
